...
gulp.task('css', () => {
    gulp.src('./src/less/**/*.less')
        ...
        // 产出的压缩后的文件名
        .pipe(rename('qtyfEditor.min.css'))
        .pipe(cssmin())
        .pipe(gulp.dest('./release'))
})
...
gulp.task('script', () => {
    ...
    return rollup.rollup({
        ...
        bundle.write({
            ...
            format: 'umd',
            // iife 规范下的全局变量名称
            moduleName: 'qtyfEditor',
            // 产出的未压缩的文件名
            dest: './release/qtyfEditor.js'
        }).then(() => {
            // 待 rollup 打包 js 完毕之后，再进行如下的处理：
            gulp.src('./release/qtyfEditor.js')
                // inline css
                .pipe(gulpReplace(/__INLINE_CSS__/gm, function () {
                    // 读取 css 文件内容
                    var filePath = path.resolve(__dirname, 'release', 'qtyfEditor.css')
